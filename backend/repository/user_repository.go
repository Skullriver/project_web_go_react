...
type UserRepository interface {
	...
	UpdateUserBalance(ctx context.Context, balance float64, userID int64) error
...
func (r *postgresUserRepository) UpdateUserBalance(ctx context.Context, balance float64, userID int64) error {
	query := "UPDATE users SET account_balance = $1 WHERE id = $2"
	_, err := r.db.ExecContext(ctx, query, balance, userID)
	if err != nil {
		return fmt.Errorf("failed to update user: %v", err)
	}
	return nil
...
func (r *postgresUserRepository) GetUserBalance(ctx context.Context, userID int64) (float64, error) {

	query := "SELECT account_balance FROM users WHERE id = $1"
